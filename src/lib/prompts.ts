...
## 評価レベル（P-Level）
合計スコアに基づいて以下のように判定：
- P1（0～59点）：設計初心者 - 設計基本概念把握段階
- P2（60～79点）：設計中級者 - Medium機能を担当可能
- P3（80～89点）：設計高級者 - Complex機能を担当可能
- P4（90～100点）：要件担当 - システム要件定義に参画可能

## フィードバック指示
${persona.feedbackStyle}

## 出力形式
合計スコア（技術+表現+論理+日本語）を計算し、以下のJSONのみを出力。説明・前置き・コードブロック記号は不要：
{"score":75,"level":"P2","technical":22,"expression":18,"logic":18,"japanese":15,"feedback":"改善点を具体的に中国語で60文字以内で記述"}`
  }
...
## 出力形式
合計スコア（技術+表現+論理）を計算し、以下のJSONのみを出力。説明・前置き・コードブロック記号は不要：
{"score":75,"level":"P2","technical":28,"expression":22,"logic":21,"feedback":"改善点を具体的に中国語で60文字以内で記述"}`
}
